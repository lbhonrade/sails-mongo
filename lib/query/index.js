...
var Query = module.exports = function Query(options, schema) {
  ...
  //Normalize projection lookup keys
  options = this.normalizeProjection(options);
...
Query.prototype.normalizeProjection = function normalizeProjection(options) {
	var self = this;
	
  if(!hop(options, 'projection')) {
  	options.projection = {};
  	return options;
  }
  ...
  // Move the id lookup from `id` to `_id`
  if(hop(options.projection, 'id') && !hop(options.projection, '_id')) {
    options.projection['_id'] = _.clone(options.projection.id);
